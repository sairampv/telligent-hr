	...
	public UserDetails loadUserByUsername(String userName)
		...
		try {
			...
			if(user.getUserName() !=null && user.getUserName().equalsIgnoreCase(userName)){
				...
				userDetails = new TelligentUser(user.getUserName(), bouncyCastleEncryptor.decryptString(user.getPassword()), true,true,true, true, authorities, true, "", user.getEmailId(),user.getEmployeeId(),"",user.getRole(),user.isChangePassword(),user.getPictureBase64());				
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally{
